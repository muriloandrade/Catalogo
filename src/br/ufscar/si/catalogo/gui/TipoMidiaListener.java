...
public class TipoMidiaListener implements ActionListener
{
	public void actionPerformed(ActionEvent event)
	{
		...
		{
			frame.clicaBotaoTipo(((JRadioButtonMenuItem) event.getSource()).getActionCommand());
		}
		else
		{
			String actionCommand = ((JRadioButton) event.getSource()).getActionCommand();

			frame.selecionaBotaoTipoMenu(actionCommand);
			...
			if (actionCommand.equals("Todos"))
			{
				...
				lblTotalMidias.setText("Total: " + todasMidias.size()
						+ (todasMidias.size() == 1 ? " mídia" : " mídias"));
